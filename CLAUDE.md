...
- `04-data-merged/mgo-glycated-proteins.tsv` — the canonical list: one row per accession, carrying the provenance of every dataset that reported it (study, cell line, system, evidence class, and the label each paper printed). Built by `merge_final_datasets.py`
...
Per-dataset provenance (study, cell line, system, evidence class) lives in that script's `PROVENANCE` table, mirroring [01-data-extracted/MANIFEST.md](01-data-extracted/MANIFEST.md); a new dataset must be added there or the merge refuses to run. **Read the `Evidence` column before drawing conclusions.** Irshad HMEC-1 and Ashour PDLF cell-free are lysates spiked with exogenous MG, not adducts formed in living cells, and are far deeper for that reason — 25 of the 43 ribosomal hits are cell-free only, and just 18 have any in-cell support.
